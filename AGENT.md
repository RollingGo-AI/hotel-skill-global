...
2. Mandatory Pre-Authorization: After confirming the environment, you must execute `node scripts/rgg.js whoami` to check the login status before calling any specific business commands. If the result indicates "Not logged in", execute `node scripts/rgg.js login` to get the authorization link. Extract the full link starting with `https` and send it directly to the user as a text link. Pause all subsequent workflows until the user confirms successful authorization.
...
3. Clear and Aesthetic Formatting: Use lists and bold formatting to organize key data.
   - When displaying hotel lists: clearly list the hotel name, star rating, distance from search location, reference price, and highlight tags.
   - When displaying room lists: list room name, bed type, total price for the stay dates and average nightly price, and free cancellation deadline.
4. Payment Guidance & Service Follow-up: After the user selects a room type, guide them via conversation to lock the price and place the order. After an order is generated, clearly provide the payment link and remind them to complete payment within 30 minutes.
